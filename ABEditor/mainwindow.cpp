...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow) {
    ...
    connect(&tokenizer, SIGNAL(nextRecord(SignatureRecord*, int)),
            SLOT(takeNextRecord(SignatureRecord*, int)));
    connect(this, SIGNAL(writeRecord(SignatureRecord*)), &tokenizer, SLOT(writeRecord(SignatureRecord*)));
    connect(&tokenizer, SIGNAL(readFinished()), this, SLOT(readFinished()));
    connect(&tokenizer, SIGNAL(writeFinished()), this, SLOT(writeFinished()));
}
...
void MainWindow::showOpenDialog() {
    ...
    if (fileDialog.exec()) {
        ...
        statusProgressBar->setValue(0);
        int status = tokenizer.prepareForRead();
        if (status == -1) {
