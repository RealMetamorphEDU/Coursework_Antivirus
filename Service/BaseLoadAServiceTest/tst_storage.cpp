...
void Storage::test_search() {
    ...
    SearchInstance* instance = storage->startSearch();
    QVERIFY(instance->isSearching());
    QByteArray ba("hello");
    QVector<SignatureRecord*> records = instance->search((byte*) ba.data(), ba.length());
    QCOMPARE(records.count(), 1);
    QVERIFY(!instance->isSearching());
    instance->resetSearch();
    QVERIFY(instance->isSearching());
    ba = "YOUR";
    records = instance->search((byte*) ba.data(), ba.length());
