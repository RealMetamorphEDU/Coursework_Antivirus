...
void Pipe::initTestCase() {
	...
	scanStatus = new ScanStatusMessage(true, 69, "C:\\Papka\\File.exe", 0, 0, 2, this);
	startScan = new StartScanMessage("C:\\Papka\\DrugoyFile.zip", true, this);
	stopScan = new StopScanMessage(this);
	pauseScan = new PauseScanMessage(this);
...
void Pipe::test_send_recieve_1msg() {
	...
	QCOMPARE(recievedMsg->getTasksLeft(), 2);
}
...
void Pipe::test_send_recieve_allmsgs() {
	...
	MonitoredDirectoriesMessage *rMonitoredDirectories = qvariant_cast<MonitoredDirectoriesMessage*>(spy1->takeFirst().at(0));
